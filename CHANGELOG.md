...
- 补丁卡片新增本地图片缓存：按「版本号 + 正文哈希」命中复用，缓存目录 `data/cache/patch_cards/`
- `/tbznews` 中的补丁附加输出改为与 `/tbzpatch` 统一的图片卡片渲染逻辑（不再仅输出截断文字）
- 补丁正文分段增强：优先按「通用更新 + 各英雄更新」拆分卡片（杜利/朱尔斯/马克/皮格马利翁/斯黛拉/凡妮莎）

### Improvements

- 补丁卡片渲染链路统一走内存缓存 + 本地磁盘缓存，版本不变时避免重复渲染
- 移除 `bazaar_get_news` LLM 工具方法（工具层仅保留纯文本数据查询能力，避免与图片输出能力边界冲突）
- 同步清理人格预设中的 `bazaar_get_news` 工具说明和工具绑定
- README 中 AI 工具列表同步更新

## v1.1.5

### Fixes

- 修复 `/tbzpatch` 输出被截断的问题：改为完整正文返回，超长内容自动拆分多张图片卡片
- 修复补丁正文混入样式/CSS/页脚等非文本内容的问题，新增清洗规则过滤噪音
- `/tbzpatch` 默认优先图片返回；渲染失败时回退为分段文本完整发送
